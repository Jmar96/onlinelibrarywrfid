...
<?php
	...
	if($epr=='save'){
		...
		$userStatus=$_POST['textust'];
		$retrieve=$_POST['textret'];

		//$userPic=$_POST['textpic'];
		
		if ($memberId==''||$password==''||$fullname==''||$contactNo==''||$emailAddress==''||$retrieve==''){
			$errMSG = "<center>You must fill in all the fields.</center>";
		}
		else{
			$a_sql=mysql_query("INSERT INTO member values('','$memberId',$password','$fullname','$address','$contactNo','$emailAddress','$userStatus','$retrieve','')");
			if ($a_sql){
				$successMSG = "<center>New record successfully added!</center>";
				header("refresh:1;memberecs.php");
			}
			else{
	...
	if($epr=='delete'){
		$id=$_GET['id'];
		$delete=mysql_query("DELETE from member where id='$id'");
		if ($delete){
	...
	if($epr=='saveup'){
		...
		if ($memberId==''||$password==''||$fullname==''||$contactNo==''||$emailAddress==''||$retrieve==''){
			...
			$a_sql=mysql_query("UPDATE member SET memberId='$memberId',password='$password',fullname='$fullname',address='$address',contactNo='$contactNo',emailAddress='$emailAddress',userStatus='$userStatus',retrieve='$retrieve' WHERE id='$id'");
			if ($a_sql){
...
<div class="main_content">
	<div class="menu">
		...
		<li><a class="current" href="../adminmodule/memberecs.php">MEMBER USER LIST</a> </li>
		<li><a href="../adminmodule/booklist.php">BOOK LIST</a></li>
		<li><a href="../adminmodule/editform.php">REPORTS</a></li>
		</ul>
	</div>
</div>
...
if($epr=='update'){
$id=$_GET['id'];
$row=mysql_query("SELECT * FROM member where id='$id'");
...
?>  
<form method="POST" action='memberecs.php?epr=saveup'>
	<table align="center">
		<tr>
			<td></td>
		...
		<tr>
			<td>Member ID:</td>
			<td><input type='text' name='textadnid' value="<?php echo $st_row['memberId'] ?>"/></td>
		</tr>
		<tr>
			<td>Password:</td>
		...
		<tr>
			...
			<input type="radio" name="textstat" value="active">Active
			<input type="radio" name="textstat" <?php if (isset($userStatus) && $userStatus="deactive") echo "checked";?> value="deactive" checked>Deactive
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<input type='button' class="btn btn-warning btn-small" name='back' value='CANCEL' onClick="window.location='../adminmodule/memberecs.php';" /></td>
		</tr>
	</table>
</form>
<?php }else{
?>
<h3 align="center">Add new Member</h3>
<?php
	if(isset($errMSG)){
			?>
            <div class="alert alert-danger">
...
?>  
<form method="POST" action='memberecs.php?epr=save'>
	<table align="center">
		<tr>
			<td>Member ID:</td>
		...
		<tr>
			<td>Status :</td>
			<td align='center'><input type='text' value="deactive" name='textust' readonly></td>
		</tr>
		<tr>
			<td>Retrieving Phrase:</td>
...
<table id="rounded-corner" width="80%">
	...
	if(isset($_POST['search1'])){
		$search=$_POST['search'];
		$stmt=mysql_query("SELECT * from member where memberId like '%{$search}%' || password like '%{$search}%' || fullname like '%{$search}%' || address like '%{$search}%' || contactNo like '%{$search}%' || emailAddress like '%{$search}%' limit $offset,$rowsperpage");
		if (mysql_num_rows($stmt) > 0){
			while ($row = mysql_fetch_array($stmt)) {
				echo "<tr>
					...
					<td align='center'>
					<a href='memberecs.php?epr=update&id=".$row['id']."'>UPDATE</a> |
					<a onClick=\"javascript: return confirm('Please confirm deletion');\"href='memberecs.php?epr=delete&id=".$row['id']."'>DELETE</a>
					</td>
				</tr>";
			}
		}else{
	...
	$stmt = mysql_query("SELECT * FROM member ORDER BY id DESC limit $offset,$rowsperpage");
	if (mysql_num_rows($stmt) > 0){
			while ($row = mysql_fetch_array($stmt)) {
			echo "<tr>
				<td>".$row['memberId']."</td>
