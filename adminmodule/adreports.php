...
<form method="POST" action='adreports.php?epr=saveup'>
	<table align="center">
		...
		<tr>
			<td>From:</td>
			<td><input type='text' name='textfr' value="<?php echo $st_row['reportFrom'] ?>"/></td>
		</tr>
		<tr>
			<td>Date:</td>
		...
		<tr>
			...
			<input type='button' class="btn btn-warning btn-small" name='back' value='CANCEL' onClick="window.location='../adminmodule/adreports.php';" /></td>
		</tr>
	</table>
</form>
<?php }else{
...
<form method="POST" action='adreports.php?epr=save'>
	<table align="center">
		<tr>
			<td>From:</td>
			<td><input type='text' name='textf' value="<?php echo $_SESSION['userName']?>"></td>
		</tr>
		<!-- <tr>
			<td>Date:</td>
...
<table id="rounded-corner" width="80%">
	...
	if(isset($_POST['search1'])){
		$search=$_POST['search'];
		$stmt=mysql_query("SELECT * from reports where reportFrom like '%{$search}%' || daytime like '%{$search}%' || status like '%{$search}%'  limit $offset,$rowsperpage");
