...
<?php
	...
	if($epr=='save'){
		...
		if ($libId==''||$username==''||$password==''||$fullname==''||$contactNo==''||$emailAddress==''||$retrieve==''){
			$errMSG = "<center>You must fill in all the fields.</center>";
		}
		else{
			$a_sql=mysql_query("INSERT INTO librarian values('','$libId','$username','$password','$fullname','$address','$contactNo','$emailAddress','$userStatus','$retrieve','')");
			if ($a_sql){
				$successMSG = "<center>New record successfully added!</center>";
				header("refresh:1;librmodule.php");
			}
			else{
	...
	if($epr=='delete'){
		$id=$_GET['id'];
		$delete=mysql_query("DELETE from librarian where id='$id'");
		if ($delete){
	...
	if($epr=='saveup'){
		...
		if ($libId==''||$username==''||$password==''||$fullname==''||$contactNo==''||$emailAddress==''||$retrieve==''){
			...
			$a_sql=mysql_query("UPDATE librarian SET libId='$libId',username='$username',password='$password',fullname='$fullname',address='$address',contactNo='$contactNo',emailAddress='$emailAddress',userStatus='$userStatus',retrieve='$retrieve' WHERE id='$id'");
			if ($a_sql){
...
if($epr=='update'){
$id=$_GET['id'];
$row=mysql_query("SELECT * FROM librarian where id='$id'");
$st_row=mysql_fetch_array($row);	
?>

<h3 align="center">Update Librarian</h3>
<?php
	if(isset($errMSG)){
			?>
...
?>  
<form method="POST" action='librmodule.php?epr=saveup'>
	<table align="center">
		<tr>
			<td></td>
		...
		<tr>
			<td>Librarian ID:</td>
			<td><input type='text' name='textadnid' value="<?php echo $st_row['libId'] ?>"/></td>
		</tr>
		<tr>
			<td>Username:</td>
		...
		<tr>
			...
			<input type='button' class="btn btn-warning btn-small" name='back' value='CANCEL' onClick="window.location='../adminmodule/librmodule.php';" /></td>
		</tr>
	</table>
</form>
<?php }else{
?>
<h3 align="center">Add new Librarian</h3>
<?php
	if(isset($errMSG)){
			?>
            <div class="alert alert-danger">
...
?>  
<form method="POST" action='librmodule.php?epr=save'>
	<table align="center">
		<tr>
			<td>Librarian ID:</td>
...
<table id="rounded-corner" width="80%">
	...
	if(isset($_POST['search1'])){
		$search=$_POST['search'];
		$stmt=mysql_query("SELECT * from librarian where username like '%{$search}%' || password like '%{$search}%' || fullname like '%{$search}%' || address like '%{$search}%' || contactNo like '%{$search}%' || emailAddress like '%{$search}%' limit $offset,$rowsperpage");
		if (mysql_num_rows($stmt) > 0){
			while ($row = mysql_fetch_array($stmt)) {
				echo "<tr>
					...
					<td align='center'>
					<a href='librmodule.php?epr=update&id=".$row['id']."'>UPDATE</a> |
					<a onClick=\"javascript: return confirm('Please confirm deletion');\"href='librmodule.php?epr=delete&id=".$row['id']."'>DELETE</a>
					</td>
				</tr>";
			}
		}else{
	...
	$stmt = mysql_query("SELECT * FROM librarian ORDER BY id DESC limit $offset,$rowsperpage");
	if (mysql_num_rows($stmt) > 0){
			while ($row = mysql_fetch_array($stmt)) {
			echo "<tr>
				<td>".$row['libId']."</td>
