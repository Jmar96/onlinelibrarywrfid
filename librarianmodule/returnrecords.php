...
<div class="main_content">
	<div class="menu">
		...
		<li><a class="current" href="../librarianmodule/returnrecords.php">RETURN BOOKS</a></li>
		</ul>
	</div>
</div>
...
<div class="container">
	...
	<div class="page-header">
    	<h1 class="h2">ALL RETURN BOOKS <!-- <a class="btn btn-default" href="../librarianmodule/addnew.php"> <span class="glyphicon glyphicon-plus"></span> &nbsp;add new </a> --> </h1> 
    </div>
...
if(isset($_POST['search1'])){
	$search = $_POST['search'];
	$stmt = $DB_con->prepare("SELECT * FROM returnbook WHERE rfidNumber like '%{$search}%' || returnBy like '%{$search}%' || isbn like '%{$search}%' || bookTitle like '%{$search}%' || dateReserve like '%{$search}%' || dateIssued like '%{$search}%' || dateReturn like '%{$search}%' ORDER BY id DESC");
	$stmt->execute();
	if($stmt->rowCount() > 0)
	{
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
		{
			...
			<!--	<img src="../book_images/<?php echo $row['bookPic']; ?>" class="img-rounded" width="250px" height="250px" /> -->
				<p class="page-header"><label class="control-label"><?php echo "RFID : " .$rfidNumber; ?></label></p>
				<p class="page-header"><label class="control-label"><?php echo "RESERVE BY : " .$returnBy; ?></label></p>
				<p class="page-header"><label class="control-label"><?php echo "ISBN : " .$isbn; ?></label></p>
				<p class="page-header"><?php echo "BOOK TITLE : ". $bookTitle; ?></p>
				<p class="page-header"><label class="control-label"><?php echo "DATE RESERVE : ". $dateReserve; ?></label></p>
				<p class="page-header"><label class="control-label"><?php echo "DATE ISSUED : " .$dateIssued; ?></label></p>
				<p class="page-header">
				<p class="page-header"><label class="control-label"><?php echo "DATE RETURN : ".$dateReturn; ?></label></p>
				<p class="page-header">
				<span>
	...
	$stmt = $DB_con->prepare('SELECT * FROM returnbook ORDER BY id DESC');
	$stmt->execute();
