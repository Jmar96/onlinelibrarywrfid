	...
	if($epr=='save'){
		...
		if ($memberId==''||$password==''||$fullname==''||$contactNo==''||$emailAddress==''||$retrieve==''){
			...
			if ($a_sql){
				...
				header("refresh:1;memberlist.php");
			}
			else{
...
<div class="main_content">
	<div class="menu">
		...
		<li><a class="current" href="../librarianmodule/memberlist.php">MEMBER USER LIST</a> </li>
...
?>  
<form method="POST" action='memberlist.php?epr=saveup'>
	<table align="center">
		<tr>
			<td></td>
		...
		<tr>
			...
			<input type='button' class="btn btn-warning btn-small" name='back' value='CANCEL' onClick="window.location='../librarianmodule/memberlist.php';" /></td>
		</tr>
	</table>
</form>
<?php }else{
...
?>  
<form method="POST" action='memberlist.php?epr=save'>
	<table align="center">
		<tr>
			<td>Member ID:</td>
...
<table id="rounded-corner" width="80%">
	...
	if(isset($_POST['search1'])){
		$search=$_POST['search'];
		$stmt=mysql_query("SELECT * from member where password like '%{$search}%' || fullname like '%{$search}%' || address like '%{$search}%' || contactNo like '%{$search}%' || emailAddress like '%{$search}%' limit $offset,$rowsperpage");
		if (mysql_num_rows($stmt) > 0){
			while ($row = mysql_fetch_array($stmt)) {
				echo "<tr>
					...
					<td align='center'>
					<a href='memberlist.php?epr=update&id=".$row['id']."'>UPDATE</a> |
					<a onClick=\"javascript: return confirm('Please confirm deletion');\"href='memberlist.php?epr=delete&id=".$row['id']."'>DELETE</a>
					</td>
				</tr>";
			}
		}else{
