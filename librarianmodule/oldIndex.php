...
if(isset($_POST['search1'])){
	$search = $_POST['search'];
	$stmt = $DB_con->prepare("SELECT * FROM books WHERE isbn like '%{$search}%' || bookName like '%{$search}%' || author like '%{$search}%' || category like '%{$search}%' || description like '%{$search}%' ORDER BY id DESC");
	$stmt->execute();
	if($stmt->rowCount() > 0)
	{
	...
	if($stmt->rowCount() > 0)
	{
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
		{
			...
			<div class="col-xs-3">
				<img src="../book_images/<?php echo $row['bookPic']; ?>" class="img-rounded" width="250px" height="250px" />
				<p class="page-header"><?php echo $isbn?></p>
				<p class="page-header"><label class="control-label"><?php echo $bookName; ?></label></p>
				<p class="page-header"><label class="control-label"><?php echo $description; ?></label></p>
				<p class="page-header"><label class="control-label"><?php echo $bookStatus; ?></label></p>
				<p class="page-header">
				<span>
