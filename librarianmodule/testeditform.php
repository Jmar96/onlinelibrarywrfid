...
?>
	...
	{
		$id = $_GET['edit_id'];
		$stmt_edit = $DB_con->prepare('SELECT * FROM booktitle WHERE id =:uid');
...
//		$isbn = $_POST['isbn'];// isbn
		$bookId = $_POST['bookId'];
		$bookTitle = $_POST['bookTitle'];// title
		$author = $_POST['author'];
		...
		if(!isset($errMSG))
		{
			$stmt = $DB_con->prepare('UPDATE booktitle 
									     SET bookId=:bbkid, 
										     bookTitle=:btit, 
										     author=:baut,
										     category=:bcat,
...
<div class="container">
	...
	<div class="page-header">
    	<h1 class="h2">Update Book. <a class="btn btn-default" href="../librarianmodule/testindex.php"> See all books </a></h1>
    </div>
